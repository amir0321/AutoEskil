...
import { Plus, Edit, ChevronDown, Users, BadgeDollarSign } from "lucide-react";
...
import SellRequestsTab from "../components/admin/SellRequestsTab";
...
export default function Admin() {
  ...
  useEffect(() => {
    setPageSeo({
      title: `Admin – ${content.brand.name}`,
      description: "Administrationspanel för AutoEskil.",
      canonical:
        ...
        <div className={styles.pageHeader}>
          <div className={styles.pageHeaderCard}>
            <h1 className={styles.pageTitle}>
              {content.admin.titleLead}{" "}
              <span className="text-accent">{content.admin.titleAccent}</span>
            </h1>
            <p className={styles.pageSubtitle}>{content.admin.subtitle}</p>
            <div className={styles.headerMeta}>
              <span className={styles.headerPill}>AutoEskil Admin</span>
              <span className={styles.headerPill}>
                Aktiv flik:{" "}
                {content.admin.tabs.find((t) => t.id === tab)?.label}
              </span>
              <span className={styles.headerPill}>
          ...
          <div className={styles.tabPanel}>
            ...
            {tab === "dealers" && (
              <DealersTab
                ...
                onDealerDeleted={fetchDealers}
              />
            )}
            {tab === "sell" && <SellRequestsTab />}
          </div>
        </div>
      </div>
    </div>
