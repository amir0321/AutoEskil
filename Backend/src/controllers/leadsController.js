...
export async function findMatchingCars(db, leadData) {
    ...
    preferred_fuel_type = preferred_fuel_type || null;
    ...
    try {
        ...
        return matchingCars.filter(car => car.match_score > 0).map(car => car.id);
    } catch (error) {
...
export async function createCarMatches(db, leadId, carIds) {
    ...
    const stmt = await db.prepare("INSERT INTO car_matches (lead_id, car_id) VALUES (?, ?)");
