...
async function getAllDealers(db) {
    try {
        const dealers = await db.all("SELECT * FROM dealers");

        // Hämta bilar med deras bilder
        ...
        const dealersWithCars = dealers.map(dealer => {
            const dealerCars = cars
                ...
                }); // Filtrerar bort dealer_id för en renare output
...
async function updateDealer(db, dealerId, dealerData) {
    ...
    try {
        ...
        const fields = Object.entries({ name, contact_person, email, phone })
        ...
        const result = await db.run(`UPDATE dealers SET ${setClause} WHERE id = ?`, params);
...
async function deleteDealer(db, dealerId) {
    try {
        ...
        const cars = await db.all("SELECT id FROM cars WHERE dealer_id = ?", [dealerId]);
