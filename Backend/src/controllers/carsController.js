...
async function addCar(db, carData) {
    const { dealer_id, brand, model, year, price, mileage, fuel_type, description } = carData;
    ...
    try {
        await db.run(`
            INSERT INTO cars (id, dealer_id, brand, model, year, price, mileage, fuel_type, description, image_url)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [id, dealer_id, brand, model, year, price, mileage, fuel_type, description, primaryImage]);

        if (imageList.length > 0) {
            for (const imgUrl of imageList) {
                await db.run(`INSERT INTO car_images (car_id, image_url) VALUES (?, ?)`, [id, imgUrl]);
            }
        }
...
async function updateCar(db, carId, carData) {
    try {
        ...
        const allowedKeys = ['dealer_id', 'brand', 'model', 'year', 'price', 'mileage', 'fuel_type', 'description', 'image_url'];
        
        const fields = Object.entries(fieldsToUpdate)
            .filter(([key, value]) => value !== undefined && allowedKeys.includes(key));
        ...
        if (fields.length > 0) {
            ...
            const result = await db.run(`UPDATE cars SET ${setClause} WHERE id = ?`, params);
        ...
        if (updateImages) {
            ...
            if (images.length > 0) {
                for (const imgUrl of images) {
                    await db.run(`INSERT INTO car_images (car_id, image_url) VALUES (?, ?)`, [carId, imgUrl]);
                }
            }
        }
