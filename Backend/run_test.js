...
async function test() {
    ...
    await db.run("INSERT OR IGNORE INTO dealers (id, name, contact_person, email, phone) VALUES ('test_dealer', 'Test', 'Test', 'test@test.com', '123')");
    ...
    if (result.success) {
        ...
        if (cars[0] && cars[0].car_images_list) {
            console.log("Parsed images:", JSON.parse(cars[0].car_images_list).filter(url => url !== null));
        }
